...
## macOS блокирует запуск MAMP

**Симптом:** при первом запуске macOS пишет, что MAMP нельзя открыть, или приложение не запускается.

**Решение:**

1. **Системные настройки** → **Конфиденциальность и безопасность** → **Всё равно открыть** (для MAMP)
2. Если кнопки нет — снова запустите MAMP из Программ, затем вернитесь в настройки
3. Подробнее — [шаг 2, установка MAMP](02-mamp.md)
