...
PDF_FILE="$OUT_DIR/03-wordpress-hosting.pdf"
CSS_FILE="$ROOT/scripts/pdf/guide.css"
COVER_FILE="$ROOT/scripts/pdf/cover-hosting.html"
HTML_FILE="$TMP_DIR/hosting.html"
MERMAID_DIR="$TMP_DIR/mermaid-hosting"
...
python3 "$ROOT/scripts/pdf/prepare-hosting.py"

echo "==> Rendering mermaid diagrams..."
node "$ROOT/scripts/pdf/render-mermaid.js" "$MERMAID_DIR"

echo "==> Building HTML..."
pandoc "$TMP_DIR/hosting.md" \
  --standalone \
  --toc \
  --toc-depth=2 \
  --css "$CSS_FILE" \
  ...
  --metadata lang=ru \
  --metadata title="WordPress на хостинге с нуля" \
  -o "$HTML_FILE"
