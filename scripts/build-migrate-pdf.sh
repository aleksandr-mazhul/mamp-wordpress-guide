...
PDF_FILE="$OUT_DIR/02-wordpress-migrate.pdf"
CSS_FILE="$ROOT/scripts/pdf/guide.css"
COVER_FILE="$ROOT/scripts/pdf/cover-migrate.html"
HTML_FILE="$TMP_DIR/migrate.html"
MERMAID_DIR="$TMP_DIR/mermaid-migrate"
...
python3 "$ROOT/scripts/pdf/prepare-migrate.py"

echo "==> Rendering mermaid diagrams..."
node "$ROOT/scripts/pdf/render-mermaid.js" "$MERMAID_DIR"

echo "==> Building HTML..."
pandoc "$TMP_DIR/migrate.md" \
  --standalone \
  --toc \
  --toc-depth=2 \
  --css "$CSS_FILE" \
  ...
  --metadata lang=ru \
  --metadata title="Перенос WordPress с localhost на хостинг" \
  -o "$HTML_FILE"
